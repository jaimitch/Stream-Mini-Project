...
public class Main {
    ...
    public static void countTueWedThur(List<Entry> entries) {
        ...
        long count1;
        // write stream
        count1 = entries.stream().filter(entry -> entry.getDay() == Day.TUESDAY ||
                entry.getDay() == Day.WEDNESDAY ||
                entry.getDay() == Day.THURSDAY).count();
...
//       // write stream
        weekdays = entries.stream().filter(entry -> entry.getDay() != Day.SATURDAY || entry.getDay() != Day.SUNDAY).map(entry -> entry.getDay().toString()).collect(Collectors.toSet());
        System.out.println(weekdays);
        System.out.println();
    }
    ...
    public static void printDurationGreaterThan10(List<Entry> entries){
        ...
        // write stream
        entries.stream().filter(entry -> entry.getDuration() > 10).forEach(entry -> entry.getDuration());
        System.out.println();
    }
...
//        temp = entries.stream().mapToInt().max().getAsInt();
        System.out.println("The Max Duration is: " + temp);
        System.out.println();
    }
    ...
    public static void listGreaterThan50(List<Entry> entries){
        ...
        System.out.println("Stream, filter, collect:");
        // write stream
        entries.stream().filter(entry -> entry.getDuration() > 50).collect(Collectors.toSet());
        System.out.println(greaterThan50);
        System.out.println();
    }
}
